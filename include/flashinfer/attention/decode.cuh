...
// Trait to detect INT4 packed V mode.
// When DTypeV == uint8_t, V is stored as packed INT4 (2 values per byte).
// FP8 uses __nv_fp8_e4m3 or __nv_fp8_e5m2, not uint8_t.
template <typename T>
struct is_int4_packed_v : std::false_type {};

template <>
struct is_int4_packed_v<uint8_t> : std::true_type {};

template <typename T>
inline constexpr bool is_int4_packed_v_v = is_int4_packed_v<T>::value;
...
#pragma unroll
  ...
  // INT4 packed: load vec_size/2 bytes (64 bits) per thread via
  // 64-bit cp.async (8-byte aligned). Non-INT4: sizeof(DTypeV)*vec_size*8.
  constexpr uint32_t vec_bits_v = int4v ? (vec_size / 2 * 8) :
      (sizeof(DTypeV) * vec_size * 8);
#pragma unroll
...
__device__ __inline__ void BatchDecodeWithPagedKVCacheDevice(const Params& params, uint8_t smem[],
  ...
  constexpr size_t k_tile_bytes = kv_tile_elems * sizeof(DTypeK);
  // INT4 packed: each byte stores 2 values, so V needs half the bytes.
  ...
  constexpr size_t v_head_bytes = int4v ? (head_dim / 2) :
  ...
  constexpr size_t v_scales_smem_bytes = int4v ? (num_tile_tokens * (head_dim / 32) * sizeof(half)) : 0;
...
#pragma unroll
    ...
    if constexpr (int4v) {
      ...
      update_local_state_int4v<vec_size, bdx, bdy * tile_size_per_bdx>(
          ...
          s, stage_idx, st, tx, head_dim, params.paged_kv.v_group_size);
    } else {
      update_local_state<vec_size, bdx, bdy * tile_size_per_bdx>(
...
cudaError_t BatchDecodeWithPagedKVCacheDispatched(Params params, typename Params::DTypeO* tmp_v,
  ...
  DISPATCH_GQA_GROUP_SIZE(num_qo_heads / num_kv_heads, GROUP_SIZE, {
    ...
    constexpr uint32_t min_kv_elem_size = sizeof(DTypeK) < sizeof(DTypeV) ? sizeof(DTypeK) : sizeof(DTypeV);
    constexpr uint32_t tile_size_per_bdx = GROUP_SIZE == 1 ? (min_kv_elem_size == 1 ? 2U : 4U) : 1U;
    DISPATCH_COMPUTE_CAP_DECODE_NUM_STAGES_SMEM(compute_capacity, NUM_STAGES_SMEM, {
      // Asymmetric smem: K tiles at sizeof(DTypeK) per element, V tiles at
      // sizeof(DTypeV) per element.  When DTypeK == DTypeV this simplifies
      // to the pre-asymmetric formula.
