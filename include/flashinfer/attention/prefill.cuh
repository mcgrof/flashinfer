...
struct KernelTraits {
  ...
  // pre-pass expands it (adding the rotated K-projection bias) before
  // the unchanged 16-bit compute path consumes it.
  static constexpr bool K_STAGED = K_STAGED_;
  using DTypeKGmem = DTypeKGmem_;
  ...
  static constexpr uint32_t UPCAST_STRIDE_K_STAGE = HEAD_DIM_QK / upcast_size<DTypeKGmem_>();
 ...
 */
// FI-4d: produce_kv is templated on the actual dtype loaded.
// Legacy homogeneous callers infer T = KTraits::DTypeKV via the default.
// Asymmetric V-side callers deduce T = DTypeV from &v_ptr.
...
// FI-4d: page_produce_kv is templated on the actual dtype loaded.
// Legacy callers infer T = KTraits::DTypeKV via the default.
// Asymmetric V-side callers deduce T = DTypeV from kv_ptr.
...
__device__ __forceinline__ void page_produce_kv(typename KTraits::SharedStorage* smem_storage,
                                                ...
                                                const uint32_t lane_idx) {
  // NOTE: for fp8, this function doesn't work for head_dim = 64 at the moment
  // FI-4d: build smem from the side-specific buffer.  In legacy
  // homogeneous mode k_smem and v_smem have the same dtype so this
  // is bit-identical; in asym mode the two branches differ in dtype
  // and the ternary on raw pointers is rejected by the type system.
  // Each branch produces the same smem_t<SWIZZLE_MODE_KV> wrapper.
  smem_t<KTraits::SWIZZLE_MODE_KV> smem =
      produce_v ? smem_t<KTraits::SWIZZLE_MODE_KV>(smem_storage->v_smem)
...
// Staged pre-bias K producer.  Identical addressing to the 128B-swizzle
// K branch of page_produce_kv, but the destination is the one-byte
// low bytes of the 16-bit K region, addressed with the one-byte cache
// stride instead of the 16-bit shared-memory stride.
template <typename KTraits>
__device__ __forceinline__ void page_produce_k_staged(
    typename KTraits::SharedStorage* smem_storage, uint32_t* smem_offset,
    typename KTraits::DTypeKGmem* k_ptr, const uint32_t kv_idx_base,
    const size_t* thr_local_kv_offset, const uint32_t kv_len, const uint32_t warp_idx,
    const uint32_t lane_idx) {
  using DType = typename KTraits::DTypeKGmem;
  static_assert(KTraits::SWIZZLE_MODE_KV == SwizzleMode::k128B,
                "staged pre-bias K requires the 128B swizzle");
  ...
  constexpr uint32_t UPCAST_STRIDE = KTraits::UPCAST_STRIDE_K_STAGE;
  uint32_t kv_idx = kv_idx_base + warp_idx * 4 + lane_idx / 8;
  static_assert(NUM_MMA_KV * 4 % NUM_WARPS_Q == 0);
#pragma unroll
  for (uint32_t i = 0; i < NUM_MMA_KV * 4 / NUM_WARPS_Q; ++i) {
    DType* gptr = k_ptr + thr_local_kv_offset[i];
#pragma unroll
    for (uint32_t j = 0; j < NUM_MMA_D / (8 / sizeof(DType)); ++j) {
      smem.load_128b_async<fill_mode>(*smem_offset, gptr, kv_idx < kv_len);
      *smem_offset = smem.template advance_offset_by_column<8>(*smem_offset, j);
...
// Staged pre-bias K expansion pre-pass.  Reads the one-byte K tile the
// producer parked in the K region, converts it to the 16-bit dtype the
// QK loop consumes, and (when a coefficient tensor is bound) adds the
// rotated K-projection bias while the values sit in registers.  The
// sincos work therefore lives in a shared-memory pre-pass — the slot
// where the rotate-at-read pass measured 2-3% — and never enters the
// MMA dependency chain.  Each thread owns one 16-element column chunk
// across rows, so the per-column frequency vector is computed once.
template <typename KTraits, bool HAS_PREBIAS>
__device__ __forceinline__ void k_smem_stage_convert(
...
__device__ __forceinline__ void k_smem_inplace_apply_rotary(
  ...
  using DTypeKV = DTypeK;  // back-compat alias
  static_assert(sizeof(DTypeK) == 2,
                "K-side RoPE requires 2-byte K dtype (BF16/FP16).");
...
#pragma unroll
    for (uint32_t mma_kv = 0; mma_kv < KTraits::NUM_MMA_KV; ++mma_kv) {
      // FI-5: K-side FP8 dequant path.  In asym K16/V8 mode K is BF16
      // so the constexpr is false and we take the BF16 branch below.
      if constexpr (sizeof(typename KTraits::DTypeK) == 1) {
...
#pragma unroll
    ...
    // FI-5: K-side column stride uses K dtype.
    if constexpr (sizeof(typename KTraits::DTypeK) == 1) {
      if (mma_d % 2 == 1) {
        *k_smem_offset_r =
  ...
  *k_smem_offset_r -= KTraits::NUM_MMA_D_QK * sizeof(typename KTraits::DTypeK);
}
...
#pragma unroll
    for (uint32_t mma_d = 0; mma_d < KTraits::NUM_MMA_D_VO; ++mma_d) {
      uint32_t b_frag[4];
      // FI-5: V-side FP8 dequant path.  In asym K16/V8 mode V is FP8
      // so this branch is taken; K BF16 path is unaffected.
...
#pragma unroll
      ...
      // FI-5: V-side column stride uses V dtype.
      if constexpr (sizeof(typename KTraits::DTypeV) == 1) {
        if (mma_d % 2 == 1) {
          *v_smem_offset_r =
...
__device__ __forceinline__ void SinglePrefillWithKVCacheDevice(
  ...
  if constexpr (std::is_same_v<DTypeQ, nv_bfloat16>) {
    ...
    // FI-4d: K-side and V-side aliases.  In legacy homogeneous
    // mode DTypeK == DTypeV == DTypeKV; in asym mode they differ.
...
__device__ __forceinline__ void BatchPrefillWithPagedKVCacheDevice(
  ...
  if constexpr (std::is_same_v<DTypeQ, nv_bfloat16>) {
    ...
    float* lse = params.lse;
    bool* block_valid_mask = params.block_valid_mask;
    // FI-4d: bind paged_kv with side-specific dtypes so K may be
    // BF16 while V is FP8.  In legacy homogeneous mode DTypeK == DTypeV
    // and this is bit-identical to the previous one-arg form.
    ...
    // offset arrays so the V loads are not misaligned by the K stride.
    ...
    uint32_t k_smem_offset_r = k_smem.template get_permuted_offset<UPCAST_STRIDE_K>(
             ...
             v_smem_offset_r = v_smem.template get_permuted_offset<UPCAST_STRIDE_V>(
                 get_warp_idx_kv<KTraits>(tid.z) * NUM_MMA_KV * 16 + lane_idx % 16, lane_idx / 16),
             // Staged pre-bias K: the producer writes the one-byte staging
             // region, so its write offset uses the staging stride.
             k_smem_offset_w =
                 KTraits::K_STAGED
                     ? smem_t<SWIZZLE_MODE_KV>::template get_permuted_offset<
                           KTraits::UPCAST_STRIDE_K_STAGE>(
                           warp_idx * KV_THR_LAYOUT_ROW + lane_idx / KV_THR_LAYOUT_COL,
                           lane_idx % KV_THR_LAYOUT_COL)
...
#pragma unroll
    for (uint32_t i = 0;
      ...
      thr_local_k_offset[i] = paged_kv.protective_get_kv_offset(
          ...
          (lane_idx % KV_THR_LAYOUT_COL) * upcast_size<DTypeK>(), last_indptr);
      thr_local_v_offset[i] = paged_kv.protective_get_kv_offset(
          page_iter, kv_head_idx, entry_idx,
          (lane_idx % KV_THR_LAYOUT_COL) * upcast_size<DTypeV>(), last_indptr);
    }
    if constexpr (KTraits::K_STAGED) {
      page_produce_k_staged<KTraits>(&smem_storage, &k_smem_offset_w, paged_kv.k_data, 0,
...
#pragma unroll
      for (uint32_t i = 0;
        ...
        paged_kv.page_size.divmod(packed_page_iter_base + warp_idx * KV_THR_LAYOUT_ROW +
                                      ...
                                      KV_THR_LAYOUT_ROW * NUM_WARPS_Q * NUM_WARPS_KV * i,
                                  page_iter, entry_idx);
        // FI-4d: split K and V offsets by side-specific dtype.
        thr_local_k_offset[i] = paged_kv.protective_get_kv_offset(
            page_iter, kv_head_idx, entry_idx,
            (lane_idx % KV_THR_LAYOUT_COL) * upcast_size<DTypeK>(), last_indptr);
        thr_local_v_offset[i] = paged_kv.protective_get_kv_offset(
      ...
      block.sync();
      if constexpr (KTraits::K_STAGED) {
        page_produce_k_staged<KTraits>(&smem_storage, &k_smem_offset_w, paged_kv.k_data,
...
cudaError_t BatchPrefillWithPagedKVCacheDispatched(Params params, typename Params::DTypeO* tmp_v,
  ...
  FLASHINFER_CUDA_CALL(cudaDeviceGetAttribute(&max_smem_per_sm,
                                              cudaDevAttrMaxSharedMemoryPerMultiprocessor, dev_id));
  // Staged pre-bias K: modules that carry prebias coefficients over a
  // one-byte K cache expand K to the 16-bit query dtype in shared
  ...
  const int num_ctas_per_sm =
      max_smem_per_sm >=
      2 * (CTA_TILE_Q * HEAD_DIM_QK * sizeof(DTypeQ) +
           (HEAD_DIM_QK * K_SMEM_BYTES + HEAD_DIM_VO * sizeof(DTypeV)) * 16 *
               NUM_WARPS_KV)
          ? 2
          : 1;
  const int max_smem_per_threadblock = max_smem_per_sm / num_ctas_per_sm;
  ...
  const uint32_t max_num_mma_kv_smem =
      (max_smem_per_threadblock - CTA_TILE_Q * HEAD_DIM_QK * sizeof(DTypeQ)) /
      ((HEAD_DIM_QK * K_SMEM_BYTES + HEAD_DIM_VO * sizeof(DTypeV)) * 16 * NUM_WARPS_KV);
