...
void BatchPrefillWithRaggedKVCacheRun(TensorView float_workspace_buffer,
  ...
  DISPATCH_context(
      ...
      RaggedParams, PagedParams, [&] {
        ...
        // FI-4d: use side-specific dtypes so K may be BF16 while V
        // is FP8.  In legacy homogeneous mode DTypeK == DTypeV and
        // these casts are bit-identical to the old DTypeKV form.
...
void BatchPrefillWithPagedKVCacheRun(TensorView float_workspace_buffer,
  ...
  DISPATCH_context(
      ...
      RaggedParams, PagedParams, [&] {
        ...
        params.q = static_cast<DTypeQ*>(q.data_ptr());
        // FI-4d: construct paged_kv with side-specific dtypes.  In
        // legacy homogeneous mode DTypeK == DTypeV and this is bit-
        // identical to the old paged_kv_t<DTypeKV, IdType> form.
